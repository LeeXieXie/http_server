...
#本地测试：python httpserver.py 8000
...
import shutil  # 对目录和文件的一般常用操作
import mimetypes  # 映射文件名到MIME类型
import re
import signal
...
class MyBaseHTTPRequestHandler(BaseHTTPRequestHandler):
    server_version = "our_http_server>>>" + __version__
    ...
    def buildTree(self, url):
        ...
        for file in files:
            if not file.startswith('.'):  # .开头的是隐藏文件
                myfile = url + "/" + file
                size_str = bytes_conversion(myfile)
                if os.path.isfile(myfile):  # 文件类型
                    self.mylist.append(str(self.myspace) + "|____" + file + " " + size_str + "\n")
                elif os.path.isdir(myfile):  # 目录类型
                    self.mylist.append(str(self.myspace) + "|____" + file + "\n")
                    # 如果有子目录可以用空格分开
    ...
    def getAllFilesList(self):
        ...
        # dirs：Prints out subdirectories from root.
        # files：Prints out all files from root and directories.
        for root, dirs, files in os.walk(translate_path(self.path)):
            files.sort()
            for file in files:
                display_name = os.path.join(root, file)
                # 删除前面的n个字符，取出相对当前目录的路径
                relative_path = display_name[len(os.getcwd()):].replace('\\', '/')[1:]
    ...
    def do_GET(self):
        ...
        if len(plist) > 2 and plist[1] == "delete":
            ...
            if os.path.exists(result):
                print("删除文件/文件夹===>", result)
                if os.path.isdir(result):
                    shutil.rmtree(result)
                else:
                    os.remove(result)
                time.sleep(0.5)  # 0.5秒后重定向
        ...
        if fd:
            ...
            if path == "/":
                ...
                self.writeList(self.mytreefile)
            fd.close()
    ...
    def do_POST(self):
        ...
        # 每次提交post请求之后更新目录树文件
        self.buildTree(translate_path(self.path))
        self.writeList(MyBaseHTTPRequestHandler.mytreefile)

    def deal_post_data(self):
        boundary = self.headers["Content-Type"].split("=")[1].encode('ascii')
        print("边界>>>", boundary)
        remain_bytes = int(self.headers['content-length'])
        print("还剩字节数>>>", remain_bytes)
        ...
        while boundary in line and str(line, encoding="utf-8")[-4:] != "--\r\n":
            ...
            try:
                ...
                return False, "不能创建文件去写，请确认你是否有权限去写！"
        ...
        for name in list_dir:
            ...
            if not display_name.startswith('.'):
                ...
                if os.path.isdir(fullname):
                    ...
                    f.write(
                        b"<td><a style='border: solid 1px red; text-decoration: none; background: red; color: white;' href=\"/delete/%s\">delete</a>" %
                        escape(fullname).encode('utf-8'))
                    f.write(
                        b"<td><a style='border: solid 1px red; text-decoration: none; background: red; color: white;' href=\"/complie/%s\">complie</a>" %
                        escape(fullname).encode('utf-8'))
                    f.write(b"</tr>")
...
def bytes_conversion(file_path, total_size=-1):
    # 计算文件大小，并且把大小动态的转化为KB，MB，GB等等
    # param file_path:文件路径
    # param total_size: 当前目录总大小
    # return 返回格式化大小
...
def main():
    ...
    httpd = HTTPServer(server_address, MyBaseHTTPRequestHandler)
    server = httpd.socket.getsockname()
    print("Server Version: " + MyBaseHTTPRequestHandler.server_version +
          ", Python Version: " + MyBaseHTTPRequestHandler.sys_version)
    print("Serving HTTP On: " +
          str(server[0]) + ", Port: " + str(server[1]) + "Running")
    httpd.serve_forever()
